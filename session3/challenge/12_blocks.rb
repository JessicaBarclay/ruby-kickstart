...
def pay_by(order)
  ...
  order.compute_tax
  yield # yield allows one to reuse something but in a different format
  order.ship_goods
end

def pay_by_visa(order, ccn)
  pay_by order do
    order.payment :type => :visa, :ccn => ccn ; order.verify_payment
  end
end
...
# each method now calls pay_by to do the repetitve code.
# then goes onto calling individual block for user needs

# could this be made into a class...
