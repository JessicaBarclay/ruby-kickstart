...
class BeerSong
	...
	def print_song
		beers.downto 1 do |i| # get number of beers and print_stanza down to 1
			print_stanza i
		end
	end
  ...
  def print_stanza(n)
    if n.zero?
      ...
      puts "#{to_eng n} #{bottle n} of beer on the wall,"        ,
           "#{to_eng n} #{bottle n} of beer,"                    ,
           "Take one down, pass it around,"                         ,
           "#{to_eng n - 1} #{bottle n-1} of beer on the wall."
    end
  end
	...
	def to_eng(n)
      ...
      "#{to_eng n/10*10}-#{to_eng n%10}".downcase
    end.capitalize
  end
end
